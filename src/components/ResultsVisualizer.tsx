...
const ResultsVisualizer: React.FC<ResultsVisualizerProps> = ({
  ...
  // Derived values
  const average = submissions.length > 0 ? submissions.reduce((a, b) => a + b, 0) / submissions.length : 0;
  const twoThirdsAverage = Math.round((average * 2) / 3);
  const closestSubmission = submissions.reduce((prev, curr) =>
    Math.abs(curr - twoThirdsAverage) < Math.abs(prev - twoThirdsAverage) ? curr : prev,
    submissions[0] || 0
  );
  ...
  const withdrawPrize = async () => {
    ...
    try {
      ...
      if (btn) { btn.textContent = "Processing..."; btn.setAttribute("disabled", ""); }
      ...
      const btn = document.getElementById("withdraw-prize-button");
      if (btn) { btn.textContent = "Withdraw Prize"; btn.removeAttribute("disabled"); }
    }
  };
        ...
        <CardContent>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
            ...
            <div className="bg-gray-900/80 p-4 rounded-lg border border-gray-700">
              ...
              <p className="text-2xl font-bold text-green-400">{twoThirdsAverage}</p>
            </div>
          </div>
          <div className="mt-4 text-center text-gray-300 bg-blue-900/30 p-3 rounded-lg">
            <span className="text-lg font-bold text-blue-300">Winning Number: {winningNumber}</span>
          </div>
        </CardContent>
      </Card>
        ...
        <Card className="bg-gradient-to-r from-yellow-900/70 to-amber-900/70 border-yellow-600 overflow-hidden">
          <CardContent className="p-6 relative">
            <div className="flex flex-col items-center text-center z-10 relative">
              ...
              <h2 className="text-2xl font-bold text-white mb-1">Winner Announcement</h2>
              <div className="flex items-center space-x-2 mb-4">
                <Sparkles className="h-5 w-5 text-yellow-300" />
                <h3 className="text-xl font-bold text-yellow-300">{formatAddress(winner)}</h3>
                <Sparkles className="h-5 w-5 text-yellow-300" />
              </div>
              <div className="bg-black/30 rounded-lg p-4 mb-4 w-full max-w-md">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-yellow-200/70 text-sm">Closest Submission</p>
                    <p className="text-xl font-bold text-white">{closestSubmission}</p>
                  </div>
                  <div>
                    <p className="text-yellow-200/70 text-sm">Prize Amount</p>
                    <p className="text-xl font-bold text-white">{prizeAmount}</p>
                  </div>
                </div>
              </div>
              {winner.toLowerCase() === account?.toLowerCase() ? (
                <Button id="withdraw-prize-button" onClick={withdrawPrize} className="bg-green-600 hover:bg-green-500 text-white flex items-center gap-2">
                  Withdraw Prize <Coins className="h-4 w-4" />
                </Button>
              ) : (
                <Button onClick={onPlayAgain} className="bg-yellow-600 hover:bg-yellow-500 text-white flex items-center gap-2">
